...
import hinata from "../assets/narutoImg/hinataRUn.gif";
import kakashi from "../assets/narutoImg/kakashi-unscreen.gif";
...
function Game(props) {
  ...
  const [gameCharacter, setGameCharacter] = useState(naruto);

  
  let [hscore,setHScore] = useState(0)
  const storedHighScore = localStorage.getItem('HighScore');

  useEffect(() => {
    setHScore(storedHighScore || 0);
  }, []);
...
}, [score]); // Dependency array should be inside the parentheses




  
  // console.log(`%c ${gameCharacter}`,'background: #222; color: #bada55')



  useEffect(()=>{
    if(props.character === 'naruto'){
        setGameCharacter(naruto)
    }else if(props.character === 'hinata'){
      setGameCharacter(hinata)
    }else if(props.character === 'kakashi'){
      setGameCharacter(kakashi)
    }
  },[props.character])
